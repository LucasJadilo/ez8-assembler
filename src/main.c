/**
 * @file main.c
 * @brief Main source file of the EZ8 Assembler.
 * @author Lucas Jadilo
 */
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[1], "--version")) {
        ...
        printf("EZ8 Assembler"
               "\nBuild v%s (%s-%02u-%02ld-%s-%s-%s) for %s"
               "\nEZ8 Library v%s"
               "\nCopyright (C) 2020-%s Lucas Jadilo"
               "\nMIT License <https://spdx.org/licenses/MIT.html>"
               "\n",
               APP_VERSION, &date_str[7], month_index + 1, strtol(&date_str[4], NULL, 10), time_str,
               &time_str[3], &time_str[6], PLATFORM, ez8_version, &date_str[7]);

        return 0;
    }

    return ez8_compile_assembly(argv[1], NULL);
}
